...
UNET_MODELS=(
    "https://huggingface.co/QuantStack/Wan2.2-S2V-14B-GGUF/resolve/main/Wan2.2-S2V-14B-Q8_0.gguf"
)
...
function provisioning_start() {
    ...
    if [[ "$DOWNLOAD_MODE" == "parallel" ]]; then
        ...
        curl -X POST https://api.liroai.com/v1/instance/update -H "Authorization: Bearer $LIRO_TOKEN" -d "last_ping=$(date '+%Y-%m-%d %H:%M:%S')"
        provisioning_download_parallel "${MODELS_DIR}/text_encoders" "${TEXT_ENCODERS[@]}"
        curl -X POST https://api.liroai.com/v1/instance/update -H "Authorization: Bearer $LIRO_TOKEN" -d "last_ping=$(date '+%Y-%m-%d %H:%M:%S')"
        provisioning_download_parallel "${MODELS_DIR}/audio_encoders" "${AUDIO_ENCODERS[@]}"
        curl -X POST https://api.liroai.com/v1/instance/update -H "Authorization: Bearer $LIRO_TOKEN" -d "last_ping=$(date '+%Y-%m-%d %H:%M:%S')"
    else
        ...
        curl -X POST https://api.liroai.com/v1/instance/update -H "Authorization: Bearer $LIRO_TOKEN" -d "last_ping=$(date '+%Y-%m-%d %H:%M:%S')"
        provisioning_get_files "${MODELS_DIR}/audio_encoders" "${AUDIO_ENCODERS[@]}"
        curl -X POST https://api.liroai.com/v1/instance/update -H "Authorization: Bearer $LIRO_TOKEN" -d "last_ping=$(date '+%Y-%m-%d %H:%M:%S')"
    fi
